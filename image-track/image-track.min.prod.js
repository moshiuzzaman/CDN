...
function ah(s, e, t, a, r) {
    const n = y(s, "shape", "raggedTensorToTensor", "int32"),
        ...
        l = a.map((c, d) =>
            y(c, `tensors${d}`, "raggedTensorToTensor", "int32")
        ),
        p = {
            shape: n,
            values: u,
...
async function Dh(s, e, t = 1) {
    ...
    return (
        s !== a && a.dispose(), e !== r && r.dispose(), q(m, r.shape, "bool")
    );
}
const Kn = Dh;
/**
            ...
            t == null &&
                console.warn(
                    `Metric for EarlyStopping ${
                        this.monitor
                    } is not available. Available metrics are: ${Object.keys(
                        e
                    )}`
                ),
            t
...
class Qg {
    ...
    gather(e, t) {
        ...
        return (
            x(this.elementShape, a[0].shape, "TensorArray shape mismatch: "),
            se(a, 0)
        );
    }
    /**
...
class Q {
    ...
    popBack(e, t) {
        ...
        const a = ie(this.elementShape, this.tensors, e),
            r = this.tensors.pop();
        return (
            (r.kept = !1), x(r.shape, e, "TensorList shape mismatch: "), v(r, a)
        );
    }
    /**
...
const eb = async (s, e, t) => {
    switch (s.op) {
        ...
        case "Switch": {
            ...
            return (
                r.kept || (r = H(r)),
                (await a.data())[0] ? [void 0, r] : [r, void 0]
            );
        }
        case "Merge": {
    ...
    switch (s.op) {
        case "Conv1D": {
            const r = i("stride", s, e, t),
                ...
                o = i("dilation", s, e, t);
            return [
                a.conv1d(i("x", s, e, t), i("filter", s, e, t), r, n, u, o),
            ];
        }
        case "Conv2D": {
    ...
    switch (s.op) {
        case "NonMaxSuppressionV5": {
            const {
                ...
                c = await r.image.nonMaxSuppressionWithScoreAsync(
                    n,
                    u,
                    o,
                    l,
        ...
        case "NonMaxSuppressionV4": {
            const {
                ...
                c = await r.image.nonMaxSuppressionPaddedAsync(
                    n,
                    u,
                    o,
                    l,
    ...
    switch (s.op) {
        ...
        case "Print":
            ...
            for (let m = 0; m < o.length; m++)
                console.log(
                    Array.prototype.slice.call(o[m].dataSync()).slice(0, p)
                );
            return [u];
        default:
            throw TypeError(`Node type ${s.op} is not implemented`);
    ...
    switch (s.op) {
        ...
        case "ImageProjectiveTransformV3": {
            const r = i("images", s, e, t),
                ...
                p = i("fillMode", s, e, t);
            return [
                a.image.transform(r, n, l.toLowerCase(), p.toLowerCase(), o, u),
            ];
        }
        default:
            throw TypeError(`Node type ${s.op} is not implemented`);
...
const cb = (s, e, t, a = k) => {
    switch (s.op) {
        case "RaggedGather": {
            const { outputNestedSplits: r, outputDenseValues: n } =
                a.raggedGather(
        ...
        case "SparseToDense": {
            ...
            return [
                a.sparseToDense(
                    ...
                    u,
                    n,
                    u.dtype === o.dtype ? o : a.cast(o, u.dtype)
                ),
            ];
        }
    ...
    for (; c.length > 0; ) {
        ...
        if (
            ((G(d) || Ab(d) || Eb(d)) &&
                u == null &&
                ((u = d),
                (o = u.children.map((h) => h.name).filter((h) => r.has(h)))),
            r.add(d.name),
...
function Tb(s, e) {
    ...
    for (const o of s) {
        for (const l of o.children.filter(u)) {
            if (!t.has(l.name))
                throw new ge(
                    `Child ${l.name} of node ${o.name} is unreachable.`
                );
        ...
        if (!r(o))
            for (const l of o.inputs) {
                if (!t.has(l.name))
                    throw new ge(
                        `Input ${l.name} of node ${o.name} is unreachable.`
                    );
...
class Ie {
    ...
    execute(e, t) {
        ...
        try {
            this.keepIntermediateTensors = P().getBool(
        ...
        return L(() => {
            ...
            this.keepIntermediateTensors &&
                ...
                Object.keys(e).forEach((b) => {
                    ...
                    (T[_] = e[b]),
                        (h[O] = T),
                        this.keepIntermediateTensors &&
                            (this.clonedTensorsMap[O] =
                                this.cloneTensorList(T));
                });
            ...
            for (const b of g) {
                ...
                (h[b.name] = O),
                    this.keepIntermediateTensors &&
                        (this.clonedTensorsMap[b.name] =
                            this.cloneTensorList(O)),
            ...
            return (
                this.parent == null && d.dispose(N), t.map((b) => E(b, h, d))
            );
        });
    }
            ...
            n[l] ||
                ...
                (o.op === "Merge"
                    ? o.inputNames.some((p) => !!E(p, r, a)) &&
                      ((n[l] = !0),
                      t.push({ contexts: a.currentContext, node: o }))
...
class Et {
    ...
    load() {
        ...
        return Ae(e)
            ? e.then((t) =>
                  t.getWeightStream == null
                      ? this.loadSync(t)
                      : this.loadStreaming(t)
              )
            : this.loadSync(e);
    }
    /**
    ...
    loadWithWeightMap(e, t) {
        ...
        if (
            ...
            (this.executor = new Ie(
                jt.Instance.transformGraph(a, this.signature)
            )),
    ...
    addStructuredOutputNames(e) {
        if (this.structuredOutputKeys) {
            const t = e instanceof me ? [e] : e,
                a = {};
            return (
                t.forEach((r, n) => (a[this.structuredOutputKeys[n]] = r)), a
            );
        }
        return e;
    }
    ...
    normalizeInputs(e) {
        ...
        if (!(e instanceof me) && !Array.isArray(e)) {
            const n =
                (t = this.signature) === null || t === void 0
                    ? void 0
                    : t.inputs;
            if (n != null)
                for (const u in n) {
        ...
        this.executor.dispose(),
            this.initializer &&
                ...
                this.resourceIdToCapturedInput &&
                    en(this.resourceIdToCapturedInput)),
...
class li extends gt {
    ...
    async setColumnNames() {
        ...
        if (
            (w(
                a.length === 0,
                () => "Duplicate column names found: " + a.toString()
            ),
            this.columnConfigs)
        ) {
    ...
    async iterator() {
        ...
        return (
            this.hasHeader && (e = e.skip(1)),
            e.map((t) => this.makeDataElement(t))
        );
    }
    makeDataElement(e) {
    ...
    constructor(e) {
        ...
        if (
            ...
            (this.columnTruncateLength =
                e.columnTruncateLength || this.fftSize),
    ...
    async start() {
        try {
            this.stream = await navigator.mediaDevices.getUserMedia({
                audio:
                    this.audioTrackConstraints == null
                        ? !0
    ...
    constructor(e, t) {
        if (
            ...
            if (
                ...
                const a =
                        (this.webcamConfig.resizeWidth * 1) /
